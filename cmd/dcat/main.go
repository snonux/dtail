...
func main() {
	...
	if err := config.Setup(source.Client, &args, flag.Args()); err != nil {
		fmt.Fprintf(os.Stderr, "unable to configure dcat: %v\n", err)
		os.Exit(1)
	}
	...
	runtime, err := cli.NewClientRuntime(context.Background(), profileFlags, "dcat")
	if err != nil {
		fmt.Fprintf(os.Stderr, "unable to initialize dcat runtime: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		dlog.Client.Error("Unable to create dcat client", err)
